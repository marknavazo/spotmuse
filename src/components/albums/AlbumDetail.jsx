...
import { Container, Grid, Paper, Typography, Button, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, List, ListItem, ListItemButton, ListItemText, Box, CircularProgress } from '@mui/material'
...
import { collection, addDoc, onSnapshot, query, where, doc, getDoc, setDoc, serverTimestamp } from 'firebase/firestore'
...
export default function AlbumDetail() {
  ...
  async function saveMyRating(val) {
    ...
    try {
      ...
      const ref = doc(collection(db, 'ratings'), ratingId)
      await setDoc(ref, { uid: u.uid, albumId: album.id, value: val, updatedAt: serverTimestamp() }, { merge: true })
      setRating(val)
                ...
                {[...Array(10)].map((_, i) => {
                  ...
                  return (
                    <Button 
                      ...
                      sx={{ minWidth: 28, height: 28, p:0, bgcolor: active ? '#1db954' : '#2a2a2a', color: active ? '#000' : '#ddd', borderRadius: '50%' }}
                    >
                      {val}
                    </Button>
                  )
            ...
            <Paper sx={{ p: 2, mb: 2, bgcolor: '#2a2a2a' }}>
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs>
                  ...
                  <div style={{ color: '#999', fontSize: '0.9em' }}>{album.artists?.map(a=>a.name).join(', ')}</div>
                </Grid>
              </Grid>
            </Paper>
          )}
