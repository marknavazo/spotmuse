...
export default function UserProfilePage() {
  ...
  if (loading) {
    return <Container><Typography>{t('Cargando...')}</Typography></Container>
  }
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
          ...
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">{userProfile?.residence}</Typography>
        ...
        {userAlbums.map(album => (
          <Grid item key={album.id} xs={12} sm={6} md={3}>
            <Paper sx={{ p: 2 }}>
              ...
              <Typography 
                variant="subtitle1" 
                sx={{ mt: 1, fontWeight: 'bold', cursor: 'pointer', color: '#1db954', textDecoration: 'underline' }}
                onClick={() => navigate(`/album/${album.albumId}`)}
              >
                {album.name}
              </Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                ...
                <Button 
                  variant="outlined"
                  sx={{ color: '#1db954', borderColor: '#1db954', '&:hover': { borderColor: '#1ed760' } }}
                  onClick={() => window.open(`https://open.spotify.com/album/${album.albumId}`, '_blank')}
                >
                  {t('Play')}
                </Button>
              </Box>
